...
@section('content_header')
    <h1 class="bg-orange text-white text-center">Crear Idea</h1>
@stop
    ...
    <div class="card bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <form action="{{ route('ideas.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <hr>
                            ...
                            <div class="row" style="clear: both;margin-left: 18px;">
                                <label for="" class="uppercase md:text-sm text-xs text-gray-500 font-semibold mb-1">Subir Documentos &nbsp</label>
                                <br/>
                                @if (isset($uploads))
                                    <p>-puedes subir {{ $uploads ? 4 - count($uploads)  : '4' }}-</p>
                                @else
                                    <p class="uppercase md:text-sm text-xs text-gray-500 font-semibold mb-1">Solo puedes subir maximo 4 archivos:</p>
                                @endif
                                <div class="col-12">
                               ...
                               <hr>
                               <br>
                               <a href="{{ route('ideas.index') }}" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 bg-danger focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"">Cancelar</a>
                               <button id="submit-all" type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"">Guardar</button>
